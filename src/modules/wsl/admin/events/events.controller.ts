...
export default class EventController {
  ...
  @ApiBody({ type: UpdateEventSimulationWeightsDto })
  @ApiResponse({
    description: "Success",
    type: Boolean,
    status: 200,
  ...
  @Post(":eventId/weights")
  async updateSimulationWeights(
    @Param() params: EventIdParamDto,
    @Body() payload: UpdateEventSimulationWeightsDto,
  ): Promise<boolean> {
    const result = await this.eventService.updateEventSimulationWeights(params.eventId, payload);
  ...
  async fetchEvent(@Param() params: EventIdParamDto): Promise<EventResponse> {
    ...
    return {
      ...
      weights: event.simulationWeights,
