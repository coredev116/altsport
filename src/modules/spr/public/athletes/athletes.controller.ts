...
@ApiTags("Athletes")
@Controller({
  path: `${SportsTypes.SUPERCROSS}/athletes`,
})
export default class AthleteController {
  ...
  @ApiQuery({
    name: "sortColumn",
    type: "enum",
    enum: SPRPublicStatsSortColumns,
    required: false,
  ...
  public async fetchAthletes(@Query() queryParams: AthleteListing): Promise<AthleteListingObject> {
    ...
    const parsedData = data.map((athleteRow) => {
      ...
      return {
        ...
        eventWins: Math.round(+Number(athleteRow.eventWins)),
        avgEventPlace: Math.round(+Number(athleteRow.placeMain)),
        // avgQualifyingPlace: Math.round(+Number(athleteRow.placeLCQ),
        ...
        avgPrelimsPlace: Math.round(+Number(athleteRow.placePrelim)),
        totalLastChanceRaces: Math.round(+Number(athleteRow.lcq_event_app)),
        avgLastChancePlace: Math.round(+Number(athleteRow.placeLCQ)),
        avgBestLapTime: +Number(athleteRow.bestLapTime),
      };
    });
