...
export class OddsService {
  ...
  async getMarkets(eventId: string): Promise<Partial<IOddMarkets>> {
    ...
    SELECT
      EXISTS (SELECT 1 FROM ${SportsDbSchema.MOTOCRS}."${
      OddsTableNames[PublicOddTypes.EVENT_WINNER]
    }" WHERE ${SportsDbSchema.MOTOCRS}."${
      OddsTableNames[PublicOddTypes.EVENT_WINNER]
      ...
      EXISTS (SELECT 1 FROM ${SportsDbSchema.MOTOCRS}."${
      OddsTableNames[PublicOddTypes.EVENT_EXACTA]
    }" WHERE ${SportsDbSchema.MOTOCRS}."${
      OddsTableNames[PublicOddTypes.EVENT_EXACTA]
    }"."eventId" = '${eventId}') AS "${PublicOddTypes.EVENT_EXACTA}",
