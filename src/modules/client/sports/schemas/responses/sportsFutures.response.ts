...
class FuturesTourYearsResponse {
  ...
  // @ApiProperty({ type: TourFuturesResponse, isArray: true })
  // futures: TourFuturesResponse[];
}

class FutureTourResponse {
  @ApiProperty({
    name: "id",
    type: "string",
    required: true,
...
export default class SportsFuturesResponse {
  ...
  @ApiProperty({ type: FutureTourResponse, isArray: true })
  tours: FutureTourResponse[];
}
