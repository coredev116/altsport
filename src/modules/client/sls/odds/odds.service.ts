...
import { RoundStatus, SportsDbSchema, OddsTableNames } from "../../../../constants/system";
...
export class OddsService {
  constructor(
    ...
    private readonly clientProjectionEventHeatsRepository: Repository<ClientProjectionEventHeatOutcome>,
  ...
  public async downloadEventPropBets(eventId: string): Promise<EventPropBetsDownloadResponse> {
    ...
    return {
      sport: "Skateboarding League",
      league: event?.leagueYear?.league?.name || null,
      year: event?.leagueYear?.year || null,
  ...
  public async fetchHeatOdds(eventId: string): Promise<ClientProjectionEventHeatOutcome[]> {
    const data = await this.clientProjectionEventHeatsRepository.find({
      select: {
        id: true,
        eventId: true,
        eventParticipantId: true,
  ...
  public async downloadHeatOdds(
    ...
    return {
      sport: "Skateboarding League",
      tour: event?.leagueYear?.league?.name || null,
      year: event?.leagueYear?.year || null,
      eventName: event.name,
      startDate: event.startDate,
      endDate: event.endDate,
  ...
  async getMarkets(eventId: string): Promise<Partial<IOddMarkets>> {
    ...
    SELECT
      EXISTS (SELECT 1 FROM ${SportsDbSchema.SLS}."${
      OddsTableNames[PublicOddTypes.EVENT_WINNER]
    }" WHERE ${SportsDbSchema.SLS}."${
      OddsTableNames[PublicOddTypes.EVENT_WINNER]
      ...
      EXISTS (SELECT 1 FROM ${SportsDbSchema.SLS}."${OddsTableNames[PublicOddTypes.SHOWS]}" WHERE ${
      SportsDbSchema.SLS
