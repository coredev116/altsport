...
export class OddsController {
  ...
  @Sse("odds/sse")
  sse() {
    return fromEvent(this.eventEmitter, `${SportsTypes.SKATEBOARDING}OddEvent`);
  }

  @ApiBody({ type: UpdateEventShowsDto })
  @ApiResponse({
    type: Boolean,
    status: 200,
  })
  @ApiOperation({
    summary: "Update event shows",
    description: "Update event shows",
  })
  @ApiParam({
    name: "eventId",
    description: "Id of the event.",
  })
  @Put(":eventId/shows/odds")
  async updateProjectionEventShows(
    @Body() payload: UpdateEventShowsDto,
    @Param() params: EventIdParamDto,
  ): Promise<boolean> {
    const result = await this.oddsService.updateEventShows(payload, params.eventId);
  ...
  async fetchEventShows(@Param() params: EventIdParamDto): Promise<EventShowsResponse> {
    const [eventOdds, clientEventOdd] = await Promise.all([
      this.oddsService.fetchEventShows(params.eventId),
      this.oddsService.fetchClientEventShow(params.eventId),
    ]);
  ...
  async updatePlayerHeadToHeads(
    ...
    // else if (playerHeadToHead.eventParticipantWinnerId)
    //   throw headToHeadExceptions.cannotUpdateheadToHeadPaidOut;

    const result = await this.oddsService.updatePlayerHeadToHeads(params.eventId, payload);
    // if (!playerHeadToHead.eventParticipantWinnerId) {
    //   await this.queueService.notifyEventUpdate({
    //     eventId,
    //     delaySeconds: 5,
    //     sportType: SportsTypes.SKATEBOARDING,
  ...
  @ApiOperation({
    summary: "Update Prop Bet Payout entry",
    description: "Update Prop Bet Payout entry",
  })
  @ApiParam({
    name: "eventId",
    description: "Id of the event.",
